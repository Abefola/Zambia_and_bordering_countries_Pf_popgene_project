...
setwd("C:/Users/afola/Desktop/Abefola_github/Zambia_2018_MIS_Pf_WGS_project/Fig2")
...
tab <- data.frame(Zambia_pcamaf.02$sample.id,pop, EV1, EV2, stringsAsFactors = FALSE)
write.table(tab, file=paste(name,'-PCAzambiamaf0.02.tsv', sep=""), 
            quote = F, row.names = F, sep="\t")
head(tab)

figpdf = paste("Fig3A1.pdf", sep="")
pdf(file = figpdf)
plot(EV1, EV3,xlab="PC1 (14.7%)", ylab="PC2 (13.3%)", xlim=c(-0.1, 0.1),ylim=c(-0.05, 0.05), col=col.list[as.integer(pop)], pch=19,cex=1, 
     alpha=1)
legend("topleft", legend=levels(pop), bg="transparent",pch=19, cex=1,col=col.list,text.col=col.list)
...
####################################################
# Fig2B - Structure analysis 
# The same VCF file used to generated structure admix plot using STRUCTURE software 
# Please read Readme_STRUCTUR_analysis_from_vcf document for details     
# Date 05/26/2022 
####################################################
